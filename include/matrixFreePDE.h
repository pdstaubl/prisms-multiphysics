// base class for PRISMS-PF PDE implementation
// Modified from PRISMS-PF 2.4.2
...
public:
   ...
   */
  MatrixFreePDE(userInputParameters_pf<dim>, userInputParameters_cp &);
  ...
  // Getter functions for interfacing with plasticity
  // Used by crystalPlasticity to retrieve information about the
  // MatrixFreePDE state.
  ...
  /**
   * PRISMS-MP: Moved this method to public
...
protected:
  ...
  /*Vector to store the inverse of the mass matrix diagonal.
  ...
  */
  ...
  virtual void
  explicitEquationRHS(
    variableContainer<dim, degree, VectorizedArray<double>> &variable_list,
    Point<dim, VectorizedArray<double>> q_point_loc) const = 0;
  ...
  void
  setPeriodicityConstraints(AffineConstraints<double>* constraints, const DoFHandler<dim>* dof_handler) const;
   ...
   */
  void
  setRigidBodyModeConstraints(const std::vector<int> rigidBodyModeComponents,
