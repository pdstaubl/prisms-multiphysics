...
// This calculatePlasticity.cc is the modified version of the following rate-independent
// crystal plasticity model: Mohammadreza Yaghoobi, John E. Allison, Veera
// Sundararaghavan, Multiscale modeling of twinning and detwinning behavior of HCP
// polycrystals,
//  International Journal of Plasticity, December 2019, 102653.
//
// To use this file, one should copy it into the following folder (replacing the original
// calculatePlasticity.cc inside the following folder with this new one):
//     plasticity/src/materialModels/crystalPlasticity/
//  Finaly, one should recompile PRISMS-Plasticity.
//
// This model includes a multiscale scheme to capture the twinning and detwinning
// mechanisms during cyclic loading of HCP polycrystals.
//////////////////////////////////////////////////////////////////////////
                                            ...
                                            unsigned int StiffnessCalFlag)
{
  ...
  Vector<double> initialHardeningMTwin(n_twin_systems), powerLawExpTwin(n_twin_systems),
    saturationStressVTwin(n_twin_systems);
  ...
  std::vector<unsigned int> activeTwinSystems(n_twin_systems_Size),
    tTwinFlag(n_twin_systems_Size);
  ...
  dgammadEmat1.resize(RegionSize,
                      std::vector<FullMatrix<double>>(n_twin_systems_Size, temp));
  ...
  for (unsigned int Region = 0; Region < NumberOfTwinnedRegionK + 1; Region++)
    {
      if (Region == 0)
        {
          ...
          M_alpha.reinit(n_Tslip_systems, n_Tslip_systems);
          N_alpha.reinit(n_Tslip_systems, n_Tslip_systems);
          M_alpha        = m_alpha;
          N_alpha        = n_alpha;
          alpha          = 0;
          n_slip_systems = n_Tslip_systems;
          ...
          initialHardeningMTwin.reinit(n_twin_systems);
          powerLawExpTwin.reinit(n_twin_systems);
          saturationStressVTwin.reinit(n_twin_systems);
          ...
          for (unsigned int i = 0; i < n_twin_systems; i++)
            {
              initialHardeningMTwin[i] =
                this->userInputs_cp.initialHardeningModulusTwin1[i];
        ...
        {
          n_twin_systems = 2;
          M_alpha.reinit(n_slip_systemsWOtwin + 2, n_slip_systemsWOtwin + 2);
          N_alpha.reinit(n_slip_systemsWOtwin + 2, n_slip_systemsWOtwin + 2);
          ...
          for (unsigned int i = 0; i < dim; i++)
            {
              M_alpha[n_slip_systemsWOtwin][i] =
                m_alpha[n_slip_systemsWOtwin + alpha - 1][i];
              N_alpha[n_slip_systemsWOtwin][i] =
                n_alpha[n_slip_systemsWOtwin + alpha - 1][i];
              M_alpha[n_slip_systemsWOtwin + 1][i] =
                m_alpha[n_slip_systemsWOtwin + alpha - 1][i];
              N_alpha[n_slip_systemsWOtwin + 1][i] =
                n_alpha[n_slip_systemsWOtwin + alpha - 1][i];
            }
          n_slip_systems = n_slip_systemsWOtwin + 2;

          q.reinit(n_slip_systems, n_slip_systems);
          // Here, we assumed that twinning and detwinning all have equal latent hardening
          // ratios.
          ...
          for (unsigned int i = 0; i < n_twin_systems; i++)
            {
              initialHardeningMTwin[i] =
                this->userInputs_cp
                  .initialHardeningModulusTwin1[i + (n_twin_systems_Size)];
              saturationStressVTwin[i] =
                this->userInputs_cp.saturationStressTwin1[i + (n_twin_systems_Size)];
              powerLawExpTwin[i] =
                this->userInputs_cp.powerLawExponentTwin1[i + (n_twin_systems_Size)];
            }
        }

      FullMatrix<double> FE_t(dim, dim),
        FP_t(dim, dim); // Elastic and Plastic deformation gradient
      ...
      FullMatrix<double> SCHMID_TENSOR1(n_slip_systems * dim, dim),
      ...
      FullMatrix<double> Dmat2(2 * dim, 2 * dim), TM(dim * dim, dim * dim),
        ElasticityTensor(2 * dim, 2 * dim), elasticStiffnessMatrix(2 * dim, 2 * dim);
      ...
      Vector<double> s_beta(n_slip_systems), h_beta(n_slip_systems),
        delh_beta_dels(n_slip_systems), h0(n_slip_systems), a_pow(n_slip_systems),
        s_s(n_slip_systems);
      ...
      Vector<double>     resolved_shear_tau_trial(n_slip_systems), b(n_slip_systems),
      ...
      if (this->userInputs_cp.enableOneTwinSys_Reorien)
        {
          for (unsigned int i = 0; i < n_twin_systems; i++)
            { //
              if (s_alpha_tau(n_slip_systemsWOtwin + i) >
                  10000 * saturationStressVTwin[i])
                {
      ...
      if (Region == 0)
          ...
          for (unsigned int i = 0; i < (n_twin_systems / 2); i++)
            {
              ...
              if ((resolved_shear_tau_trial[j] < 0) || (tTwinMaxFlag == 0) ||
                  ttwinvf[i] <= this->userInputs_cp.MPtwinLowerThresholdFraction1)
                { // cp_twin
                  resolved_shear_tau_trial[j] = 0;
                }
            }

          /////////////The modification compared to the original model is applied to the
          ///following lines//////////////////
          for (unsigned int i = (n_twin_systems / 2); i < n_twin_systems; i++)
            {
              unsigned int j = i + n_slip_systemsWOtwin;
              /*if ((resolved_shear_tau_trial[j] > 0) || (ttwinvf[i - (n_twin_systems /
               * 2)] <= 0.0)) //cp_twin bracket is removed*/
              if ((resolved_shear_tau_trial[j] > 0) ||
                  (ttwinvf[i - (n_twin_systems / 2)] <=
                   this->userInputs_cp.MPtwinLowerThresholdFraction1))
                {
                  resolved_shear_tau_trial[j] = 0;
                }
            }
      ...
      else
        {
          if (resolved_shear_tau_trial[n_slip_systemsWOtwin] < 0 ||
              (this->twinfraction_iter1[cellID][quadPtID][0] <=
               this->userInputs_cp.MPtwinLowerThresholdFraction1))
            { // cp_twin added twinvf condition
              resolved_shear_tau_trial[n_slip_systemsWOtwin] = 0;
            }
          /* if ((resolved_shear_tau_trial[1+n_slip_systemsWOtwin] > 0)||(tTwinMaxFlag ==
           * 0)) //cp_twin bracket is removed*/
          if ((resolved_shear_tau_trial[1 + n_slip_systemsWOtwin] > 0) ||
              (ttwinvf[0] <= this->userInputs_cp.MPtwinLowerThresholdFraction1))
            {
      ...
      /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
      ////////////////////////////////////Start Nonlinear iteration for Slip
      ///increments////////////////////////////////////
      ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
      ...
      while (iter1)
        {
          ...
          for (unsigned int i = 0; i < n_slip_systemsWOtwin; i++)
            {
              h_beta(i) = initialHardeningM[i] *
                          pow((1 - s_beta(i) / saturationStressV[i]), powerLawExp[i]);
            }
          ...
          for (unsigned int i = 0; i < n_twin_systems; i++)
            {
              h_beta(n_slip_systemsWOtwin + i) =
                initialHardeningMTwin[i] *
                pow((1 - s_beta(n_slip_systemsWOtwin + i) / saturationStressVTwin[i]),
                    powerLawExpTwin[i]);
            }
          ...
          inactive_slip_removal(active,
                                ...
                                A,
                                A_PA,
                                n_slip_systemsWOtwin); // cp
          // cp
          for (unsigned int i = 0; i < n_twin_systems / 2; i++)
            {
              // x_beta[n_slip_systemsWOtwin+i]=0.129*fabs(this->dtwinfraction_iter1[cellID][quadPtID][i])*this->delT;
              // //cp
              if (ttwinvf1[i] >
                  this->userInputs_cp.MPtwinLowerThresholdFraction1) // if(ttwinvf1[i]>twinfraction_iter[cellID][quadPtID][i])
                                                                     // //cp_trial
                x_beta[n_slip_systemsWOtwin + i] =
                  0.129 *
                  fabs(this->twinfraction_iter1[cellID][quadPtID][i] -
                       twinfraction_iter[cellID][quadPtID][i]); // cp_twinfraction_change
              else
                x_beta[n_slip_systemsWOtwin + i] = 0; // cp_twinfraction_change
              x_beta_old[n_slip_systemsWOtwin + i] =
                x_beta_old[n_slip_systemsWOtwin + i] + x_beta[n_slip_systemsWOtwin + i];
            }
          // cp
          temp.reinit(dim, dim);
                  ...
                  for (unsigned int k = 0; k < dim; k++)
                    {
                      if (resolved_shear_tau(i) > 0)
                        T_star_tau[j][k] =
                          T_star_tau[j][k] - 0.5 * x_beta(i) * temp4[j][k];
                      else
                        T_star_tau[j][k] =
                          T_star_tau[j][k] + 0.5 * x_beta(i) * temp4[j][k];
                    }
          ...
          if (Region == 0)
            {
              ...
              /////////////The modification compared to the original model is applied to
              ///the following lines//////////////////
              for (unsigned int i = (n_twin_systems / 2); i < n_twin_systems; i++)
                {
                  unsigned int j = i + n_slip_systemsWOtwin;
          ...
          else
            {
              if (resolved_shear_tau_trial[n_slip_systemsWOtwin] < 0 ||
                  (this->twinfraction_iter1[cellID][quadPtID][0] <=
                   this->userInputs_cp.MPtwinLowerThresholdFraction1))
                { // cp_try added twinvfcondition
                  resolved_shear_tau_trial[n_slip_systemsWOtwin] = 0;
                }
              // if ((resolved_shear_tau_trial[1 + n_slip_systemsWOtwin] > 0) ||
              // (tTwinMaxFlag == 0) ||(ttwinvf[1 + n_slip_systemsWOtwin] <= 0.1))
              // //bracket removed //cp_twon_check
              if ((resolved_shear_tau_trial[1 + n_slip_systemsWOtwin] > 0) ||
                  (tTwinMaxFlag == 0) ||
                  (ttwinvf[0] <= this->userInputs_cp.MPtwinLowerThresholdFraction1))
                {
              ...
              for (unsigned int i = 0; i < n_twin_systems_Size; i++)
                {
                  ...
                  if ((this->twinfraction_iter1[cellID][quadPtID][i] <= this->userInputs_cp.MPtwinLowerThresholdFraction1)
                      || (this->twinfraction_iter1[cellID][quadPtID][i] > this->userInputs_cp.MPtwinUpperThresholdFraction1)
                      || resolved_shear_tau[n_slip_systemsWOtwin + i] < 0)
                    energy[cellID][quadPtID][i] = 0.0;
                }
            }
      ...
      ////////////////////////////////////End Nonlinear iteration for Slip
      ///increments////////////////////////////////////
      ...
      for (unsigned int i = 0; i < (n_twin_systems / 2); i++)
        {
          int num_items1 =
            std::count(myPA.begin(), myPA.end(), (i + n_slip_systemsWOtwin));
          int num_items2 = std::count(myPA.begin(),
                                      myPA.end(),
                                      (i + n_twin_systems / 2 + n_slip_systemsWOtwin));
          if (num_items1 == 1)
            {
  ...
  std::vector<unsigned int> tActiveTwinSystems(NumberOfTwinnedRegionK),
    DeactiveTwinSystems(n_twin_systems_Size - NumberOfTwinnedRegionK);
  unsigned int jj = 0;
  unsigned int kk = 0;
  for (unsigned int i = 0; i < n_twin_systems_Size; i++)
    {
  ...
  Vector<double> twinvfNucleated(NumberOfTwinnedRegionK),
    RegionsTwinvf(1 + NumberOfTwinnedRegionK);
          ...
          for (unsigned int j = 0; j < dim * dim; j++)
            {
              PK1_Stiff_RVE[i][j] =
                PK1_Stiff_RVE[i][j] +
                RegionsTwinvf[Region] * PK1_Stiff_Region[Region][i][j];
            }
        }
    }
  ...
  /*for (unsigned int ii = 0;ii < NumberOfTwinnedRegionK;ii++) {
    ...
    for (unsigned int i = 0;i < dim;i++) {
      for (unsigned int j = 0;j < dim;j++) {
        dgammadEmatRegion[i][j] = ((1 - RegionsTwinvf[0])*dgammadEmat1[0][alpha-1][i][j] -
  RegionsTwinvf[1 + ii] * dgammadEmat1[1 + ii][0][i][j]) / this->userInputs_cp.twinShear1;
        UntwinnedRegionsTwinvfDiff[i][j] = UntwinnedRegionsTwinvfDiff[i][j] +
  dgammadEmatRegion[i][j];
      }
    }
  ...
  */
                              ...
                              for (unsigned int l = 0; l < dim; l++)
                                {
                                  dP_dF[m][n][o][p] =
                                    dP_dF[m][n][o][p] +
                                    PK1_Stiff_RVE(dim * i + j, dim * k + l) * L(i, m) *
                                      L(j, n) * L(k, o) * L(l, p);
                                  ;
                                }
                            }
  ...
  for (unsigned int i = 0; i < n_slip_systemsWOtwin; i++)
    {
      slipfraction_iter[cellID][quadPtID][j * n_slip_systemsWOtwin + i] =
        slipfraction_conv[cellID][quadPtID][j * n_slip_systemsWOtwin + i] +
        tslipvfsys[j][i] * RegionsTwinvf[0];
    }
  for (unsigned int i = 0; i < n_slip_systemsWOtwin; i++)
    {
      for (unsigned int j = 1; j < n_twin_systems_Size + 1; j++)
        {
          slipfraction_iter[cellID][quadPtID][j * n_slip_systemsWOtwin + i] =
            slipfraction_conv[cellID][quadPtID][j * n_slip_systemsWOtwin + i] +
            tslipvfsys[j][i] * ttwinvf[j - 1];
        }
    }

  for (unsigned int i = 0; i < n_twin_systems_Size * 2; i++)
    {
      TwinOutputfraction_iter[cellID][quadPtID][i] =
        TwinOutputfraction_conv[cellID][quadPtID][i] +
        tslipvfsys[0][n_slip_systemsWOtwin + i] * RegionsTwinvf[0];
    } // check
  ...
  for (unsigned int i = 0; i < 2; i++)
    {
      for (unsigned int j = 0; j < n_twin_systems_Size; j++)
        {
          TwinOutputfraction_iter[cellID][quadPtID][n_twin_systems_Size * 2 + j * 2 + i] =
            TwinOutputfraction_conv[cellID][quadPtID]
                                   [n_twin_systems_Size * 2 + j * 2 + i] +
            tslipvfsys[j + 1][n_slip_systemsWOtwin + i] * ttwinvf[j];
        }
    }

  for (unsigned int i = 0; i < n_twin_systems_Size; i++)
    {
      // ttwinvf[i] = ttwinvf[i] + (1 - Totaltwinvf)*(tslipvfsys[0][n_slip_systemsWOtwin +
      // i] - tslipvfsys[0][n_slip_systemsWOtwin + n_twin_systems_Size + i]) /
      // this->userInputs_cp.twinShear1; ttwinvf[i] = ttwinvf[i] +
      // fabs(this->dtwinfraction_iter1[cellID][quadPtID][i])*this->delT;
      ttwinvf[i] =
        (this->twinfraction_iter1[cellID][quadPtID][i]); // cp_check_twinvf //changed abs
                                                         // to actual twinfrac
      if (ttwinvf[i] < 0.0)
        ttwinvf[i] = 0.0; // cp
      if (ttwinvf[i] > this->userInputs_cp.MPtwinLowerThresholdFraction1)
        TwinFlag_conv[cellID][quadPtID][i] = 1; // cp_new_try
    }
  ...
  for (unsigned int i = 0; i < NumberOfTwinnedRegionK; i++)
    {
      ...
      // ttwinvf[alpha - 1] = ttwinvf[alpha - 1] * (1 -
      // (tslipvfsys[alpha][n_slip_systemsWOtwin] - tslipvfsys[alpha][n_slip_systemsWOtwin
      // + 1]) / this->userInputs_cp.twinShear1);
      if (ttwinvf[alpha - 1] < this->userInputs_cp.twinThresholdFraction1)
        {
          ...
          for (unsigned int i = 0; i < dim; i++)
            {
              rotnew_iter[cellID][quadPtID][alpha * dim + i] =
                rot[cellID][quadPtID][alpha * dim + i];
            }
          for (unsigned int i = 0; i < n_slip_systemsWOtwin; i++)
            {
              ...
              for (unsigned int k = 0; k < dim; k++)
                {
                  for (unsigned int l = 0; l < dim; l++)
                    {
                      Fe_iter[cellID][quadPtID][k][l + alpha * dim] =
                        Fe_iter[cellID][quadPtID][k][l];
                      Fp_iter[cellID][quadPtID][k][l + alpha * dim] =
                        Fp_iter[cellID][quadPtID][k][l];
                    }
                }

              for (unsigned int k = 0; k < n_slip_systemsWOtwin; k++)
                {
                  s_alpha_iter[cellID][quadPtID][n_Tslip_systems * alpha + k] =
                    s_alpha_iter[cellID][quadPtID][k];
                }
              s_alpha_iter[cellID][quadPtID]
                          [alpha * n_Tslip_systems + n_slip_systemsWOtwin] =
                            this->userInputs_cp
                              .initialSlipResistanceTwin1[n_twin_systems_Size * 2 - 1];
              s_alpha_iter[cellID][quadPtID]
                          [alpha * n_Tslip_systems + n_slip_systemsWOtwin + 1] =
                            this->userInputs_cp
                              .initialSlipResistanceTwin1[n_twin_systems_Size * 2 - 1];
              for (unsigned int k = 0; k < dim; k++)
                {
                  rotnew_iter[cellID][quadPtID][alpha * dim + k] =
                    rot[cellID][quadPtID][alpha * dim + k];
                }
              for (unsigned int k = 0; k < n_slip_systemsWOtwin; k++)
                {
                  slipfraction_iter[cellID][quadPtID][alpha * n_slip_systemsWOtwin + k] =
                    0;
                }
                  ...
                  for (unsigned int kk = 0; kk < n_twin_systems_Size; kk++)
                    {
                      if (kk != (alpha - 1))
                        {
                          s_alpha_iter[cellID][quadPtID][kk + n_slip_systemsWOtwin] =
                            s_alpha_iter[cellID][quadPtID][kk + n_slip_systemsWOtwin] *
                            1000000000;
                        }
                    }
  ...
  twinvfNucleated.reinit(NumberOfTwinnedRegionK);
  RegionsTwinvf.reinit(1 + NumberOfTwinnedRegionK);
  Totaltwinvf     = 0;
  twintemp        = 0;
  RegionsTwinvf   = 0;
  twinvfNucleated = 0;
  ...
  ///////The twin volume fraction should be updated first and then update
  ///TwinMaxFlag!!!!!!
